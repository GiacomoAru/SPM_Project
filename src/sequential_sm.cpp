...
#define DEFAULT_LOG_NAME "./log/sequential_sm.csv" // default log file name
...
int main(int argc, char *argv[]) {   
    ...
    for (int i = 0; i < matrix_size; i++){
        um.element(i, i) = (i + 1.0) / matrix_size;
    }
    ...
    for (int diag = 1; diag < um.matrix_dimension; diag++){
        ...
        for (int k = 0; k < (um.matrix_dimension - diag); k++){
            ...
            for (int i = 0; i < diag; i++){
                ...
                partial_sum += um.element(k, k + i) * um.element(diag + k - i, diag + k);
            }
            DEBUG_CHECK(5, std::cout << " |" << std::endl;)

            // Update matrix element with the cube root of the partial sum
    ...
    // Log execution times and other details to the specified file
    std::ofstream file;
