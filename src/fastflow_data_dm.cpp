...
#define DEFAULT_LOG_NAME "./log/fastflow_data_dm.csv" // default log file name
...
int main(int argc, char *argv[]) {   
    ...
    int matrix_size = DEFAULT_M_SIZE;  // Default size of the matrix
    ...
    // um_col - represents an upper triangular matrix with row-major order (the coordinates needs to be inverted)
    UpperMatrix um_row(matrix_size, DEFAULT_INITIALIZATION);
    UpperMatrix um_col(matrix_size, DEFAULT_INITIALIZATION);
    ...
    for (int diag = 1; diag < um_row.matrix_dimension; diag++) {
        long v = (um_row.matrix_dimension - diag);
        ...
        pf.parallel_for(0L, v, 1L,
                        ...
                        [&](const long k) {
                            ...
                            for (int l = 0; l < diag; l++) {
                                ...
                                // Memory order has a significant impact on execution time
                                partial_sum += um_row.element(k, k + l) * um_col.element(um_col.matrix_dimension - 1 - (diag + k), um_col.matrix_dimension - 1 - (diag + k - l));
                            }
